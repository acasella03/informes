...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.conn = sqlite3.connect('facturas.dat')
        self.cursor = self.conn.cursor()
        self.cursor.execute('''CREATE TABLE IF NOT EXISTS productos
                                      (nombre TEXT, precio REAL, cantidad INTEGER, total REAL)''')
        self.conn.commit()
        ...
        # Inicializar self.table_data llamando a update_table_data después de la inicialización de self.table_view
    ...
    def add_product(self):
        ...
        if ok_name and ok_price and ok_quantity:
            ...
            self.cursor.execute("INSERT INTO productos (nombre, precio, cantidad, total) VALUES (?, ?, ?, ?)",
    ...
    def generate_invoice(self):
        try:
            ...
            c = canvas.Canvas("ejemploFacturaExamenQSQLite.pdf", pagesize=A4)
            ...
            # Calcular el total
            total = sum(Decimal(row[3]) for row in self.table_data)
